const dataFilters = (
  ...
  switch (action.type) {
    ...
    case "RECEIVE_DATA_FILTERS":
      return {...state, ...{available: {isFetching: false, filters: action.filters }}};
    ...
    case "UPDATE_FILTER":
      ...
      let changedFilter = available.filters.find(f => f.key === action.changedFilterKey);
      changedFilter.value = action.newValue;
      
      // empty the choices and values for all children
      let firstChild = available.filters.find(f => f.parentKey === changedFilter.key);
